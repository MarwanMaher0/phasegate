#!/usr/bin/env node
import { createRequire as __phasegateCreateRequire } from 'node:module';
...
var PROFILES = ["small", "standard"];
function templatesDir(profile) {
  return join(packageRoot(), "templates", profile);
}
function parseGuide(id, markdown) {
...
function builtinGuide(id) {
  const dir = join(packageRoot(), "phases");
  if (!existsSync(dir)) return void 0;
  const file = readdirSync(dir).find((f) => f.replace(/^\d+-/, "") === `${id}.md`);
  return file ? parseGuide(id, readFileSync(join(dir, file), "utf8")) : void 0;
}
function builtinPhaseIds() {
  const dir = join(packageRoot(), "phases");
  if (!existsSync(dir)) return [];
  return readdirSync(dir).filter((f) => /^\d+-[a-z-]+\.md$/.test(f)).sort().map((f) => f.replace(/^\d+-/, "").replace(/\.md$/, ""));
}
function guideFor(id, root, guidePath) {
  if (root && guidePath) {
...
// src/commands.ts
import { existsSync as existsSync3, mkdirSync, readdirSync as readdirSync3, writeFileSync, readFileSync as readFileSync4 } from "node:fs";
...
var CONFIG_FILENAMES = ["phasegate.config.yaml", "phasegate.config.yml"];
...
var DEFAULT_MARKERS = ["TODO", "TBD", "FIXME"];
var DEFAULT_ADR_ALLOWED = ["Accepted", "Superseded"];
var DEFAULT_REQUIREMENT_PATTERN = "^REQ-\\d+";
var DEFAULT_COMMAND_TIMEOUT = 300;
var COMMON_KEYS = ["type", "level", "name", "hint"];
var GATE_KEYS = {
...
var GATE_TYPES = Object.keys(GATE_KEYS);
...
function suggest(input, options) {
  ...
  return best !== void 0 && bestDistance <= Math.max(2, Math.floor(input.length / 3)) ? ` Did you mean "${best}"?` : "";
}
function isRecord(v) {
...
function stringList(value, where, problems) {
...
function parseGate(raw, where, problems) {
  if (!isRecord(raw)) {
  ...
  const spec = GATE_KEYS[type];
  const before = problems.length;
  checkKeys(raw, [...COMMON_KEYS, ...spec.required, ...spec.optional], where, problems);
  for (const key of spec.required) {
    const v = raw[key];
    if (typeof v !== "string" || v.trim() === "") {
      problems.push(`${where}: "${key}" is required for gate type "${type}" and must be a non-empty string${v === void 0 ? "" : `, got ${describe(v)}`}.`);
    }
  }
  ...
  if (type === "acceptance-criteria") {
    try {
      ...
      problems.push(`${where}.pattern: not a valid regular expression (${e.message}).`);
    }
  }
  let timeout = DEFAULT_COMMAND_TIMEOUT;
  if (raw.timeout !== void 0) {
  ...
  if (problems.length > before) return void 0;
  const base = {
    level,
  ...
  switch (type) {
    ...
    case "section":
      return { type: "section", path, heading: raw.heading, exclude, ...base };
    ...
    case "command":
      return { type: "command", run: raw.run, timeout, ...base };
  }
}
function parseArtifact(raw, where, problems) {
...
function validateConfig(raw, file = "phasegate.config.yaml") {
  const problems = [];
  if (!isRecord(raw)) {
  ...
  if (raw.version !== 1) {
    problems.push(
      raw.version === void 0 ? 'missing "version". Add `version: 1` at the top of the file.' : `unsupported "version" ${describe(raw.version)}. This release of phasegate understands version 1.`
    );
  }
  const phases = [];
  if (!Array.isArray(raw.phases) || raw.phases.length === 0) {
    ...
    raw.phases.forEach((p, i) => {
      ...
      for (const key of ["title", "guide"]) {
        if (p[key] !== void 0 && typeof p[key] !== "string") problems.push(`${at}: "${key}" must be a string.`);
      }
      const artifacts = [];
      if (p.artifacts !== void 0 && p.artifacts !== null) {
        if (!Array.isArray(p.artifacts)) problems.push(`${at}: "artifacts" must be a list.`);
        else
          p.artifacts.forEach((a, j) => {
            const spec = parseArtifact(a, `${at}.artifacts[${j}]`, problems);
            if (spec) artifacts.push(spec);
          });
      }
      const gates = [];
      if (p.gates !== void 0 && p.gates !== null) {
  ...
  return { version: 1, current: raw.current, phases };
}
function parseConfig(text, file = "phasegate.config.yaml") {
  const doc = (0, import_yaml.parseDocument)(text, { prettyErrors: true });
  if (doc.errors.length > 0) {
...
function findConfig(cwd, explicit) {
  if (explicit) {
    const p = resolve2(cwd, explicit);
    if (!existsSync2(p)) throw new ConfigNotFoundError(p);
    return p;
  }
...
function loadConfig(cwd, explicit) {
  ...
  const config = parseConfig(readFileSync2(path, "utf8"), path);
...
function walk(root, rel, out) {
  let entries;
  try {
    entries = readdirSync2(join2(root, rel), { withFileTypes: true });
  } catch {
    return;
  }
...
function expandGlob(root, pattern, exclude = []) {
  const p = toPosix(pattern);
  if (!hasGlob(p)) {
    return isFile(join2(root, p)) && !isExcluded(p, exclude) ? [p] : [];
  }
  const segments = p.split("/");
  const firstGlob = segments.findIndex((s) => hasGlob(s));
  const base = segments.slice(0, firstGlob).join("/");
  const files2 = [];
  walk(root, base, files2);
  const re = globToRegExp(p);
  return files2.filter((f) => re.test(f) && !isExcluded(f, exclude)).sort();
}
...
function stripCode(md) {
  ...
  return lines.map((line, i) => mask[i] ? "" : line.replace(/(`+)(?:(?!\1).)+?\1/g, "")).join("\n");
}
...
function normalizeHeading(text) {
  return text.replace(/[*_`]/g, "").replace(/^\s*\d+(?:\.\d+)*[.)]?\s+/, "").replace(/[\s:.]+$/, "").replace(/\s+/g, " ").trim().toLowerCase();
}
function sectionFor(lines, headings, index, stopAt) {
  const heading = headings[index];
  let end = lines.length;
  for (let j = index + 1; j < headings.length; j++) {
    const next2 = headings[j];
    if (next2.level <= heading.level || stopAt && stopAt(next2)) {
      end = next2.line;
      break;
    }
  }
...
function findSection(md, heading) {
  ...
  return index === -1 ? void 0 : sectionFor(lines, headings, index);
}
...
function findMarkers(md, markers) {
...
function findRequirements(md, pattern) {
  const stripped = stripComments(md);
  const lines = splitLines(stripped);
  const headings = parseHeadings(stripped);
  const plain = (t) => t.replace(/[*_`]/g, "").trim();
  const isReq = (h) => pattern.test(plain(h.text));
  const reqs = [];
...
function gatePath(gate) {
  return gate.type === "command" ? void 0 : gate.path;
}
...
function read(root, rel) {
  return readFileSync3(join3(root, rel), "utf8");
}
function files(ctx, gate) {
...
function evaluateGate(gate, ctx) {
  switch (gate.type) {
    ...
    case "section": {
      ...
      return fail(problems.length === 1 ? problems[0].replace(/^[^:]+(?::\d+)?: /, "") : `${problems.length} files are missing "${gate.heading}"`, problems);
    }
    case "acceptance-criteria": {
      const found = files(ctx, gate);
      if (found.length === 0) return missing(gate.path);
    ...
    case "adr-status": {
      ...
      const bad = [];
      for (const f of found) {
        const status2 = findAdrStatus(read(ctx.root, f));
        if (!status2) bad.push(`${f}: no status found (add a "## Status" section)`);
        else if (!allowed.includes(status2.toLowerCase())) bad.push(`${f}: status is ${status2}`);
      }
...
// src/evaluate.ts
function evaluateProject(loaded, options = {}) {
  ...
  const phases = config.phases.map((phase, index) => {
    ...
    const artifacts = phase.artifacts.map((a) => {
      const files2 = expandGlob(root, a.path, a.exclude);
      return { path: a.path, present: files2.length > 0, files: files2 };
    });
...
function countsAsFailure(gate, strict) {
  return gate.status === "fail" && (gate.level === "required" || strict);
}
function check(report, options = {}) {
...
function nextStep(report) {
  const first = (p, level) => p.gates.find((g) => g.status === "fail" && g.level === level);
  const describeGate = (g) => `${g.label}: ${g.message}`;
  for (const p of report.phases.filter((ph) => ph.position === "before")) {
    const gate2 = first(p, "required");
    if (gate2) {
      return {
        kind: "skipped-phase",
        phase: p.id,
        title: p.title,
        message: `The ${p.id} phase was skipped. ${describeGate(gate2)}`,
        gate: gate2,
        ...gate2.hint ? { hint: gate2.hint } : {}
      };
    }
  }
...
// src/render.ts
var wrap = (on, open, close) => (s) => on ? `\x1B[${open}m${s}\x1B[${close}m` : s;
...
function shouldColor(flag, env, isTTY) {
  if (flag === false) return false;
  if (env.NO_COLOR !== void 0 && env.NO_COLOR !== "") return false;
  if (env.FORCE_COLOR !== void 0 && env.FORCE_COLOR !== "0" && env.FORCE_COLOR !== "false") return true;
  return isTTY;
}
var ANSI_RE = /\[[0-9;]*m/g;
...
function table(rows, gap = 2) {
  ...
  return rows.map(
    (row) => row.map((cell, i) => i === row.length - 1 ? cell : pad(cell, (widths[i] ?? 0) + gap)).join("").trimEnd()
  ).join("\n");
}
var STATE_LABEL = {
  done: "done",
...
function renderStatus(report, next2, c) {
  const out = [];
  out.push(`${c.bold("phasegate status")}  ${c.dim(report.config)}`);
  const current = report.phases.find((p) => p.position === "current");
  ...
  out.push("");
  out.push(`${c.bold("Next:")} ${next2.message}`);
  out.push(c.dim(`Why it matters: phasegate explain ${next2.phase}`));
  return out.join("\n");
}
function renderCheck(result, c) {
  const out = [];
  ...
  if (result.ok) {
    ...
    out.push(
      c.red(c.bold(`phasegate check failed: ${result.failures.length} gate${result.failures.length === 1 ? "" : "s"} in ${phases} phase${phases === 1 ? "" : "s"}`)) + (result.strict ? c.dim(" (strict)") : "")
    );
    out.push(c.dim(`Run \`phasegate explain ${result.failures[0]?.phase}\` to see why this phase matters.`));
  }
  return out.join("\n");
}
function renderNext(next2, c) {
  const out = [];
  const tag = {
  ...
  out.push(`${tag[next2.kind]} ${c.dim(`[${next2.phase}]`)}`);
  out.push(next2.message);
  if (next2.gate) for (const d of next2.gate.details) out.push(`  ${c.dim(d)}`);
  if (next2.hint) out.push(`${c.cyan("hint:")} ${next2.hint}`);
  out.push(c.dim(`Why it matters: phasegate explain ${next2.phase}`));
  return out.join("\n");
}
function renderMarkdown(md, c) {
  return md.trimEnd().split("\n").map((line) => {
    ...
    return line.replace(/\*\*(.+?)\*\*/g, (_, t) => c.bold(t)).replace(/`([^`]+)`/g, (_, t) => c.cyan(t));
  }).join("\n");
}

// src/commands.ts
var EXIT = { ok: 0, failed: 1, error: 2 };
function ok(stdout, code = EXIT.ok) {
...
function listFiles(dir, base = dir) {
  return readdirSync3(dir, { withFileTypes: true }).flatMap((e) => {
    const p = join4(dir, e.name);
    return e.isDirectory() ? listFiles(p, base) : [relative2(base, p).split("\\").join("/")];
  });
}
function init(opts) {
  ...
  for (const rel of listFiles(source).sort()) {
    const target = join4(opts.cwd, rel);
    if (existsSync3(target) && !opts.force) {
      skipped.push(rel);
      continue;
    }
    mkdirSync(dirname2(target), { recursive: true });
    writeFileSync(target, readFileSync4(join4(source, rel)));
    written.push(rel);
  }
  const out = [`phasegate init (${profile} profile)`];
  for (const f of written) out.push(`  created  ${f}`);
  for (const f of skipped) out.push(`  skipped  ${f} (already exists; use --force to overwrite)`);
  out.push("");
  out.push(
    written.length > 0 ? "Next: fill in the discover document, then run `phasegate next`. `phasegate explain discover` says why it matters." : "Nothing written: every file already exists."
  );
  return ok(out.join("\n"));
}
function status(opts) {
  return guard(() => {
    ...
    const next2 = nextStep(report);
    if (opts.json) {
      return ok(JSON.stringify({ ...report, next: next2 }, null, 2));
    }
    return ok(renderStatus(report, next2, colors(opts)));
  });
}
...
function explain(phase, opts) {
  return guard(() => {
    ...
    if (!guide && !configured) {
      const known = [.../* @__PURE__ */ new Set([...loaded?.config.phases.map((p) => p.id) ?? [], ...builtinPhaseIds()])];
...
No teaching content for this phase. Add \`guide: path/to/${phase}.md\` to it in the config.`);
    if (configured) {
      out.push("");
      out.push(c.bold(`In this project (${relative2(opts.cwd, loaded.path) || loaded.path})`));
...
program2.name("phasegate").description("Walk a project through its phases, discover to operate, and check the gates that show each phase is really done.").version(VERSION).showHelpAfterError().addHelpText(
  "after",
  "\nExit codes: 0 ok, 1 gates failing (check), 2 usage or configuration error.\nNo network access and no telemetry."
);
var withConfig = (cmd) => cmd.option("-c, --config <path>", "path to phasegate.config.yaml").option("--no-color", "disable colour (NO_COLOR is also respected)");
program2.command("init").description("write phasegate.config.yaml and docs/phases/ templates (never overwrites without --force)").addOption(new Option("-p, --profile <profile>", "template set").choices(["small", "standard"]).default("standard")).option("-f, --force", "overwrite files that already exist").action((flags) => emit(init({ cwd: process.cwd(), profile: flags.profile ?? "standard", force: Boolean(flags.force) })));
withConfig(program2.command("status")).description("show each phase: artifacts present, gates passing, and the current phase").option("--json", "machine-readable output").option("--run-commands", "also run command gates (they are skipped by default here)").action((flags) => emit(status({ ...common(flags), json: Boolean(flags.json), runCommands: Boolean(flags.runCommands) })));
withConfig(program2.command("check")).description("evaluate gates; exits 1 if any fail. Default: every phase before `current`").option("--phase <id>", "check only this phase").option("--strict", "recommended gates fail the check too").option("--json", "machine-readable output").action(
  (flags) => emit(
    checkCommand({
      ...common(flags),
...
withConfig(program2.command("explain")).argument("<phase>", "phase id, for example design").description("why a phase exists, what it produces, and how teams skip it").action((phase, flags) => emit(explain(phase, common(flags))));
withConfig(program2.command("next")).description("the single most important missing thing").option("--json", "machine-readable output").option("--run-commands", "also run command gates").action((flags) => emit(next({ ...common(flags), json: Boolean(flags.json), runCommands: Boolean(flags.runCommands) })));
...
try {
  ...
  process.stderr.write(`phasegate: ${e.stack ?? String(e)}
`);
