...
function suggest(input2, options) {
  let best;
  let bestDistance = Infinity;
  for (const option of options) {
    const d = levenshtein(input2.toLowerCase(), option.toLowerCase());
    if (d < bestDistance) {
      best = option;
      bestDistance = d;
    }
  }
  return best !== void 0 && bestDistance <= Math.max(2, Math.floor(input2.length / 3)) ? ` Did you mean "${best}"?` : "";
}
function isRecord(v) {
...
function builtinGuide(id) {
  ...
  return file ? parseGuide(id, readFileSync2(join(dir, file), "utf8")) : void 0;
}
function guideFor(id, root, guidePath) {
  if (root && guidePath) {
    const p = resolve2(root, guidePath);
    if (existsSync2(p)) return parseGuide(id, readFileSync2(p, "utf8"));
  }
...
function renderSummaryMarkdown(result) {
  ...
  const icon = (g) => g.status === "pass" ? "\u2705" : g.status === "skip" ? "\u23ED\uFE0F" : g.level === "required" || result.strict ? "\u274C" : "\u26A0\uFE0F";
  const lines = [];
...
function runAction(env, cwd, io) {
  try {
    ...
    io.log(`::error title=phasegate::${escapeData(known ? e.message : e.stack ?? String(e))}`);
